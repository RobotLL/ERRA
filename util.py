...
def render_camera(rl=False, map_real=False):

    camera_position = [0, 0, 0.4]
    camera_target = [0, 0, 0]
    camera_fov = 55.371784673413806
    camera_position[0] = camera_position[0] + random.uniform(-1, 1)*0.0025
    camera_position[1] = camera_position[1] + random.uniform(-1, 1)*0.0025
    camera_position[2] = camera_position[2] + random.uniform(-1, 1)*0.0025
    camera_target[0] = camera_target[0] + random.uniform(-1, 1)*0.0025
    camera_target[1] = camera_target[1] + random.uniform(-1, 1)*0.0025
    ...
    if map_real is True:
        B = np.arange(0, 1.005, 0.005)
        for i in range(sim_float.shape[0]):
            for j in range(sim_float.shape[1]):
                nearest_value = min(B, key=lambda x: abs(x - sim_float[i][j]))
                sim_float[i][j] = nearest_value
