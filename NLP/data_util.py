...
class NLPRLProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir: str, task, level) -> List[InputExample]: 
        img_features = np.loadtxt(f"{data_dir}{task}/task_{task}_{level}_img.csv",dtype=float,delimiter=',')
        txt_inputs = np.loadtxt(f"{data_dir}{task}/task_{task}_{level}_txt.csv",dtype=str,delimiter=',')
        tact_inputs = np.loadtxt(f"{data_dir}{task}/task_{task}_{level}_tact.csv",dtype=int,delimiter=',')
        labels = np.loadtxt(f"{data_dir}{task}/task_{task}_{level}_label.csv",dtype=str,delimiter=',')
