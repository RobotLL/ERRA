...
def collect_high_level_task_data(collect_number, output_path,c_id=None, generate_test=False):
    os.makedirs(output_path+'task_cut_saycan/', exist_ok=True)
    language_input_list = []
    image_input_list = []
    tactile_input_list = []
    label_language_list = []
    ...
    for d_id in range(collect_number):
        ...
        # plt.imshow(sim_d)
        if c_id is not None:
            img_save_path = output_path+'task_cut_saycan/' + str(int(d_id)) + '_' + str(int(case_id)) + '_high.jpg'
        elif generate_test is False:
            img_save_path = output_path+'task_cut_saycan/' + str(int(d_id)) + '_high.jpg'
        elif generate_test is True:
            img_save_path = output_path+'task_cut_saycan/' + str(int(d_id)) + 'test_high.jpg'
        ...
        if case_id == 6:
            # str_command = '1. 'Grasp the knife' 2. 'Cut the ' + object_name, 3 'Done''
            label_language_list.append(
                [low_level_command[0], low_level_command[1], low_level_command[2]])
...
def collect_low_level_task_data(collect_number, output_path,c_id=None, generate_test=False):
    ...
    for d_id in range(collect_number):
        ...
        # plt.imshow(sim_d)
        if c_id is not None:
            img_save_path = output_path+'task_cut_saycan/' + str(int(d_id)) + '_' + str(int(case_id)) + '_low.jpg'
        elif generate_test is False:
            img_save_path = output_path+'task_cut_saycan/' + str(int(d_id)) + '_low.jpg'
        elif generate_test is True:
            img_save_path = output_path+'task_cut_saycan/' + str(int(d_id)) + '_test_low.jpg'
        depth_image = Image.fromarray(sim_d).convert('L')
        depth_image.save(img_save_path)
...
if __name__ == "__main__":
    ...
    np.savetxt("sytask_cut.csv", merge_list, delimiter=", ", fmt='% s')
