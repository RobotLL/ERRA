...
def collect_high_level_task_data(collect_number, output_path, c_id=None, generate_test=False):
    os.makedirs(output_path+'task_pick_round/', exist_ok=True)
    language_input_list = []
    image_input_list = []
    tactile_input_list = []
    label_language_list = []
    ...
    for d_id in range(collect_number):
        ...
        # plt.imshow(sim_d)
        if c_id is not None:
            img_save_path = output_path +'task_pick_round/' + str(int(d_id)) + '_' + str(int(case_id)) + '_high.jpg'
        elif generate_test is False:
            img_save_path = output_path +'task_pick_round/' + str(int(d_id)) + '_high.jpg'
        elif generate_test is True:
            img_save_path = output_path +'task_pick_round/' + str(int(d_id)) + 'test_high.jpg'
...
def collect_low_level_task_data(collect_number, output_path, c_id=None, generate_test=False):
    ...
    for d_id in range(collect_number):
        case_id, object_name, obj_id_list,  tactile_info, _ = build_pick_round_env(
            obj_random=True, rl=False)
        sim_d, sim_float, mask = render_camera()
        # plt.imshow(sim_d)
        if c_id is not None:
            img_save_path = output_path +'task_pick_round/' + str(int(d_id)) + '_' + str(int(case_id)) + '_low.jpg'
        elif generate_test is False:
            img_save_path = output_path +'task_pick_round/' + str(int(d_id)) + '_low.jpg'
        elif generate_test is True:
            img_save_path = output_path +'task_pick_round/' + str(int(d_id)) + '_test_low.jpg'
        depth_image = Image.fromarray(sim_d).convert('L')
        depth_image.save(img_save_path)
...
if __name__ == "__main__":
    ...
    np.savetxt("task_pick_round.csv", merge_list, delimiter=", ", fmt='% s')
