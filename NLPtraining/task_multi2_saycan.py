...
def collect_high_level_task_data(collect_number, output_path, c_id=None, generate_test=False):
    os.makedirs(output_path+'task_multi2_saycan/', exist_ok=True)
    language_input_list = []
    image_input_list = []
    tactile_input_list = []
    label_language_list = []
    ...
    for d_id in range(collect_number):
        ...
        if c_id is not None:
            img_save_path = output_path+'task_multi2_saycan/' + str(int(d_id)) + '_' + str(int(case_id)) + '_high.jpg'
        elif generate_test is False:
            img_save_path = output_path+'task_multi2_saycan/' + str(int(d_id)) + '_high.jpg'
        elif generate_test is True:
            img_save_path = output_path+'task_multi2_saycan/' + str(int(d_id)) + 'test_high.jpg'
...
if __name__ == "__main__":
    ...
    np.savetxt("sytask_multi2.csv", merge_list, delimiter=", ", fmt='% s')
